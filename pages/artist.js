...
export default function Artist() {
	...
	const mintRightoken = async () => {
		...
		// const contract = await factory.deploy(songTitle+" : "+artistName+" : Rightoken","RTKN")
		// const customERC20RightokenAddress = contract.address
		...
		*/
		...
		/*
		let abi = ["function approve(address _spender, uint256 _value) public returns (bool success)"]
		let contract = new ethers.Contract("0xde275455A3e6c452115D2b950A8dD0432f63de56", abi, signer)
		console.log(contract.approve)
		await contract.approve(account, (12 * 10 ** 18).toString(), {from: account, gasPrice: ethers.utils.parseUnits('5', 'gwei'), gasLimit: 85000})
		*/
	
		// create a Uniswap LP (UniswapV2Router02.sol)
		const uniswapRouterContract = new ethers.Contract("0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D", IUniswapV2Router02.abi, signer)
		const liquidity = await uniswapRouterContract.addLiquidityETH("0xde275455A3e6c452115D2b950A8dD0432f63de56", (10 * 10 ** 18).toString(), (8 * 10 ** 18).toString(), (.01 * 10 ** 18).toString(), account, (Date.now() + 1000 * 60 * 10).toString(), {from: account, gasPrice: ethers.utils.parseUnits('5', 'gwei'), gasLimit: 3400000, value: (.025 * 10 ** 18).toString()})
		const receipt = await liquidity.wait()
		console.dir(receipt)
	
		/*
		addLiquidityETH (https://docs.uniswap.org/protocol/V2/reference/smart-contracts/router-02#addliquidityeth)
		{
