...
export default function Mint() {
	...
	async function mintRightokenERC20() {
		...
		try {
			if (chainId !== networkDefaults.mainnet_id && chainId !== networkDefaults.testnet_id) {
				throw `Please connect to an ${networkDefaults.mainnet_name} network`
			}
	...
	async function listRightokenERC20() {
		...
		try {
			...
			let stablecoinAddress = networkDefaults.stablecoin_mainnet_address

			if (chainId === networkDefaults.mainnet_id) {
				stablecoinAddress = networkDefaults.stablecoin_testnet_address
			}
			else if (chainId !== networkDefaults.mainnet_id && chainId !== networkDefaults.testnet_id) {
				throw `Please connect to an ${networkDefaults.mainnet_name} network`
			}
	...
	function SwitchNetworkButton(props) {
		...
		return <>
			...
			<div className="flex flex-col justify-center space-y-2">
				{ chainId !== networkDefaults.mainnet_id &&
					<button
						className={`uppercase font-bold mix-blend-multiply ${chainId === networkDefaults.testnet_id ? "text-xs text-zinc-700 px-3 py-2 text-zinc-400 active:bg-zinc-200" : "text-sm px-4 py-3 bg-gradient-to-r from-emerald-100 via-green-100 to-emerald-100 active:from-emerald-50 active:via-green-50 active:to-emerald-100"} active:text-zinc-500 rounded-md`}
						onClick={
							async () => {
								try {
									await library.provider.request({
										method: "wallet_switchEthereumChain",
										params: [{ chainId: `0x${networkDefaults.mainnet_id.toString(16)}` }]
									})
								}
								catch (e) {
									await library.provider.request({
										...
										params: [
											{
												chainId: `0x${networkDefaults.mainnet_id.toString(16)}`, // 10
												chainName: networkDefaults.mainnet_name,
												rpcUrls: [networkDefaults.mainnet_rpc_url],
												blockExplorerUrls: [networkDefaults.mainnet_block_explorer_url]
											}
										]
									})
								}
					...
					>
						Connect to {networkDefaults.mainnet_name}
					</button>
				}
				{ chainId !== networkDefaults.testnet_id &&
					<button
						...
						onClick={
							async () => {
								try {
									await library.provider.request({
										method: "wallet_switchEthereumChain",
										params: [{ chainId: `0x${networkDefaults.testnet_id.toString(16)}` }]
									})
								}
								catch (e) {
									await library.provider.request({
										...
										params: [
											{
												chainId: `0x${networkDefaults.testnet_id.toString(16)}`,
												chainName: networkDefaults.testnet_name,
												rpcUrls: [networkDefaults.testnet_rpc_url],
												blockExplorerUrls: [networkDefaults.testnet_block_explorer_url]
											}
										]
									})
								}
			...
			<br />
			{ (chainId === networkDefaults.mainnet_id || chainId === networkDefaults.testnet_id) && 
				<div className="rounded-sm bg-zinc-50 mix-blend-multiply py-2 text-center"><p className="text-green-600 font-mono text-xs"><span className="align-middle inline-block w-1 h-1 rounded-full bg-green-600 animate-ping" />  Your wallet connected to {chainId === networkDefaults.testnet_id && "test"} {networkDefaults.mainnet_name} </p></div>
			}
		</>
	}
	...
	const mintStepPages = [
		...
		},
		{
			...
			body: <><span className="font-medium">Rightoken is built on {networkDefaults.mainnet_name}, a network that makes Ethereum much cheaper to use.</span></>,
			additionalContent: <SwitchNetworkButton chainId={chainId} />,
			successCondition: (chainId === networkDefaults.mainnet_id || chainId === networkDefaults.testnet_id),
		},
		{
			title: "Fund wallet",
			body: <><span className="font-medium">You need Ethereum in your {networkDefaults.mainnet_name} wallet to pay blockchain gas fees.</span> The fees don't go to Rightoken. <br /><br /> Download the Crypto.com <a href="https://apps.apple.com/us/app/crypto-com-buy-btc-eth-shib/id1262148500" className="underline" target="_blank" rel="noreferrer">iOS</a> or <a href="https://play.google.com/store/apps/details?id=co.mona.android&hl=en&gl=US" className="underline" target="_blank" rel="noreferrer">Android</a> app, purchase at least 0.006 ETH, and withdraw to {networkDefaults.mainnet_name} using your wallet address: <span className="inline-block text-xs font-mono bg-zinc-200 rounded-sm leading-loose break-all select-all px-2 py-1">{account}</span> <br /><br /> If you have Ethereum not on {networkDefaults.mainnet_name}, you can send it to your wallet and <a href="https://app.hop.exchange/" className="underline" target="_blank" rel="noreferrer">bridge to {networkDefaults.mainnet_name}</a>, but it'll cost more in gas fees.</>,
			additionalContent: <>
					...
					<br />
					{((chainId === networkDefaults.mainnet_id || chainId === networkDefaults.testnet_id) && ethBalance > 0.005) ? 
			...
			successCondition: ((chainId === networkDefaults.mainnet_id && ethBalance > 0.005) || chainId === networkDefaults.testnet_id)
		},
		{
