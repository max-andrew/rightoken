...
export default function Mint() {
	...
	async function mintRightokenERC20() {
		...
		try {
			if (chainId !== OPTIMISM_KOVAN_CHAIN_ID && chainId !== OPTIMISM_CHAIN_ID) {
				throw "Please connect to an optimism network"
			}
	...
	function SwitchNetworkButton(props) {
		...
		return <>
							...
							async () => {
								try {
									...
									await library.provider.request({
										...
										params: [
											{
												chainId: `0x${OPTIMISM_CHAIN_ID.toString(16)}`, // 10
						...
						onClick={
							async () => {
								try {
									await library.provider.request({
										method: "wallet_switchEthereumChain",
										params: [{ chainId: `0x${OPTIMISM_KOVAN_CHAIN_ID.toString(16)}` }]
									})
								}
								catch (e) {
									await library.provider.request({
										...
										params: [
											{
												chainId: `0x${OPTIMISM_KOVAN_CHAIN_ID.toString(16)}`, // 69
			...
			{ (chainId === OPTIMISM_CHAIN_ID || chainId === OPTIMISM_KOVAN_CHAIN_ID) && 
				<div className="rounded-sm bg-zinc-50 mix-blend-multiply py-2 text-center"><p className="text-green-600 font-mono text-xs"><span className="align-middle inline-block w-1 h-1 rounded-full bg-green-600 animate-ping" />  Your wallet connected to {chainId === OPTIMISM_CHAIN_ID && "test"} Optimism</p></div>
			}
		</>
	}
	...
	const legalAgreementLibrary = {
		...
		delist: {
			title: "Delisting",
			body: "The artist or original rightsholders may delist obtstanding Rightoken ERC-20, effectively reversing the process of tokenization. This is done by crediting the addresses of holders with equal value of a reasonably fungible, recognized, and established alternative token.",
			mutable: false,
		}
	}
	...
	const mintStepPages = [
		...
		},
		{
			...
			body: <><span className="font-medium">Rightoken is built on optimism, a network that makes Ethereum much cheaper to use.</span></>,
			additionalContent: <SwitchNetworkButton chainId={chainId} />,
			successCondition: (chainId === OPTIMISM_CHAIN_ID || chainId === OPTIMISM_KOVAN_CHAIN_ID),
		},
		{
			title: "Fund wallet",
			body: <><span className="font-medium">You need Ethereum in your optimism wallet to pay blockchain gas fees.</span> The fees don't go to Rightoken. <br /><br /> Download the Crypto.com <a href="https://apps.apple.com/us/app/crypto-com-buy-btc-eth-shib/id1262148500" className="underline" target="_blank" rel="noreferrer">iOS</a> or <a href="https://play.google.com/store/apps/details?id=co.mona.android&hl=en&gl=US" className="underline" target="_blank" rel="noreferrer">Android</a> app, purchase at least 0.006 ETH, and withdraw to optimism using your wallet address: <span className="inline-block text-xs font-mono bg-zinc-200 rounded-sm leading-loose break-all select-all px-2 py-1">{account}</span> <br /><br /> If you have Ethereum not on optimism, you can send it to your wallet and <a href="https://bridge.optimism.io/" className="underline" target="_blank" rel="noreferrer">bridge to optimism</a>, but it'll cost more in gas fees.</>,
			additionalContent: <>
					...
					<br />
					{((chainId === OPTIMISM_CHAIN_ID || chainId === OPTIMISM_KOVAN_CHAIN_ID) && ethBalance > 0.005) ? 
			...
			successCondition: ((chainId === OPTIMISM_CHAIN_ID && ethBalance > 0.005) || chainId === OPTIMISM_KOVAN_CHAIN_ID)
		},
		{
