...
export default function Footer(props) {
	...
	const places = ["NYC", "Lisbon", "Tokyo", "Vienna"]
	const [place, setPlace] = useState(places[Math.floor(Math.random() * places.length)])

	return (
		<footer className="flex flex-col space-y-2 items-center justify-center w-full h-24 border-t">
			<p className="text-xs font-medium text-zinc-600" suppressHydrationWarning>Written {sadverb} in {place}</p>
